...
socsim-survey   ← 設定駆動のサーベイマイクロデータ recode: SurveySchema（人口統計ごとの valmap ＋ アウトカムマップ ＋ 年齢ビン），スキーマ拡張ポイント; recode_row / demo_label / actual_outcome / estimate_distributions; ANES 2012/2016/2020 の組み込みスキーマはオプションの `anes` feature の背後（デフォルトは汎用エンジンのみ）（socsim 依存なし; エンジン非依存; ライブラリ専用）
