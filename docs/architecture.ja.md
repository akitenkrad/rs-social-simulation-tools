...
socsim-datasets ← サーベイデータセットのスキーマ ＋ 機械可読レジストリ ＋ オプションの取得: 正規の ANES 2012/2016/2020 SurveySchema ビルダ（socsim-survey から原文どおり移動）＋ CES 2022 のメタデータスタブ; DatasetMeta / DataFile / Source レジストリ（DOI / URL / 引用 / ライセンス ＋ ファイルごとの sha256 / expect_rows）; オプションの `acquire` feature（fetch のダウンロード/キャッシュ/検証 ＋ Rust 製 raw→CSV コンバータ，データは決して同梱せず，ライセンス制約のあるファイルは Source::Manual として宣言）（→ socsim-survey; エンジン非依存; ライブラリ専用）
...
- `socsim-datasets` は**リーフかつエンジン非依存・ライブラリ専用**のクレートで，**`socsim-survey` のみ**に依存します（`SurveySchema` / `DemoVar` / `ValMap` / `AgeBins` / `OutcomeMap` 型のため）．サーベイ replication の*データセット固有*側の単一の真実源です：データセットのスキーマ（socsim-survey から原文どおり移動した正規の ANES 2012/2016/2020 `SurveySchema` ビルダ，ならびにメタデータのみの CES 2022 スタブ）と，機械可読のレジストリ（`DatasetMeta` / `DataFile` / `Source` レコード —— DOI，ソース URL，引用，ライセンス，検証用のファイルごとの `sha256` / `expect_rows`; `Source` は `Dataverse` / `Url` / `Manual`）を保持します．オプションの **`acquire` feature**（`ureq` ＋ `sha2` ＋ `csv` ＋ `tempfile` ＋ `anyhow` を取り込む）は，`fetch()`（消費側リポジトリの gitignore された `data/` ディレクトリへのアトミック書き込みダウンロード，`sha256` ＋ 行数の検証，キャッシュヒット時のスキップ付き）と `raw_to_csv()`（パイプ区切りの raw→CSV コンバータをバイト単位で一致させた Rust 移植）を追加します．データを決してリポジトリに同梱せず，ライセンス制約のあるファイル（生の ANES マイクロデータ，CES 2022）は自動ダウンロードせず `Source::Manual` として宣言します．
