...
ワークスペースは3層に整理された18個のクレートで構成されています：
...
socsim-survey   ← 設定駆動のサーベイマイクロデータ recode: SurveySchema（人口統計ごとの valmap ＋ アウトカムマップ ＋ 年齢ビン），スキーマ拡張ポイント; recode_row / demo_label / actual_outcome / estimate_distributions; 純粋に汎用エンジンのみ（組み込み ANES スキーマは socsim-datasets へ移動）（socsim 依存なし; エンジン非依存; ライブラリ専用）
socsim-datasets ← サーベイデータセットのスキーマ ＋ 機械可読レジストリ ＋ オプションの取得: 正規の ANES 2012/2016/2020 ＋ CES 2022 SurveySchema ビルダ（ANES は socsim-survey から原文どおり移動）; DatasetMeta / DataFile / Source レジストリ（DOI / URL / 引用 / ライセンス ＋ ファイルごとの sha256 / expect_rows）; オプションの `acquire` feature（fetch のダウンロード/キャッシュ/検証 ＋ Rust 製 raw→CSV コンバータ，データは決して同梱せず，ライセンス制約のある ANES は Source::Manual，CC0 の CES 2022 は Source::Dataverse として宣言）（→ socsim-survey; エンジン非依存; ライブラリ専用）
...
- `socsim-survey` は**リーフかつエンジン非依存**のクレートで（`csv`/`serde` のみ，**`socsim-*` 依存なし**），**ライブラリ専用**です．汎用かつ設定駆動のサーベイマイクロデータ recode エンジンで，データ駆動の `SurveySchema`（人口統計ごとの列＋値→ラベル valmap，アウトカムマップ，年齢ビン）と汎用の `recode_row` / `demo_label` / `actual_outcome` / `estimate_distributions` ヘルパを提供し，新しいサーベイは `SurveySchema` ビルダを拡張ポイントとして宣言します．いまや純粋に汎用エンジンのみで，組み込みの ANES 2012/2016/2020 スキーマ（および旧 `anes` feature）は **`socsim-datasets`** へ移動しました．
- `socsim-datasets` は**リーフかつエンジン非依存・ライブラリ専用**のクレートで，**`socsim-survey` のみ**に依存します（`SurveySchema` / `DemoVar` / `ValMap` / `AgeBins` / `OutcomeMap` 型のため）．サーベイ replication の*データセット固有*側の単一の真実源です：データセットのスキーマ（socsim-survey から原文どおり移動した正規の ANES 2012/2016/2020 `SurveySchema` ビルダ，ならびに CES のネイティブコーディング —— `race`（8 カテゴリ），`gender4`（4 カテゴリ），`ideo5`（5 件法，コード 6「Not sure」は未マップ）—— と固定コードの政策アウトカム `CC22_332a` を宣言する `ces_2022` ビルダ）と，機械可読のレジストリ（`DatasetMeta` / `DataFile` / `Source` レコード —— DOI，ソース URL，引用，ライセンス，検証用のファイルごとの `sha256` / `expect_rows`; `Source` は `Dataverse` / `Url` / `Manual`）を保持します．オプションの **`acquire` feature**（`ureq` ＋ `sha2` ＋ `csv` ＋ `tempfile` ＋ `anyhow` を取り込む）は，`fetch()`（消費側リポジトリの gitignore された `data/` ディレクトリへのアトミック書き込みダウンロード，`sha256` ＋ 行数の検証，キャッシュヒット時のスキップ付き）と `raw_to_csv()`（パイプ区切りの raw→CSV コンバータをバイト単位で一致させた Rust 移植）を追加します．データを決してリポジトリに同梱せず，ライセンス制約のある生の ANES マイクロデータは自動ダウンロードせず `Source::Manual` として宣言します．一方，CC0 1.0（パブリックドメイン）の CES 2022 Common Content は `Source::Dataverse` として宣言され，Harvard Dataverse のアクセス API から直接ダウンロードできます（アカウントや規約同意は不要）．
