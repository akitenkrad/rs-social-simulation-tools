...
socsim-datasets ← survey dataset schemas + machine-readable registry + optional acquisition: canonical ANES 2012/2016/2020 + CES 2022 SurveySchema builders (ANES moved here verbatim from socsim-survey); DatasetMeta / DataFile / Source registry (DOI / URL / citation / license + per-file sha256 / expect_rows); optional `acquire` feature (fetch download/cache/verify + Rust raw→CSV converter, never vendors data; license-gated ANES declared Source::Manual, CC0 CES 2022 declared Source::Dataverse) (→ socsim-survey; engine-free; library-only)
