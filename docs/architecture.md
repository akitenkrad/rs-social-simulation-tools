...
The workspace contains seventeen crates organised in three layers:
...
socsim-survey   ← config-driven survey microdata recode: SurveySchema (per-demographic valmap + outcome map + age bins), built-in ANES 2012/2016/2020 schemas, CES extension point; recode_row / demo_label / actual_outcome / estimate_distributions (no socsim deps; engine-free; library-only)
...
- `socsim-survey` is a **leaf, engine-free** crate (only `csv`/`serde`; **no `socsim-*` dependencies**) and **library-only**. It generalises the hard-coded per-year ANES recode used by the `sun2024` replication into a data-driven `SurveySchema` (per-demographic column + value→label valmap, an outcome map, and age bins), shipping built-in ANES 2012/2016/2020 schemas plus a CES extension point, with generic `recode_row` / `demo_label` / `actual_outcome` / `estimate_distributions` helpers.
