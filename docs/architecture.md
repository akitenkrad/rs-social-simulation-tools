...
socsim-survey   ← config-driven survey microdata recode: SurveySchema (per-demographic valmap + outcome map + age bins), schema extension point; recode_row / demo_label / actual_outcome / estimate_distributions; built-in ANES 2012/2016/2020 schemas behind the optional `anes` feature (default = generic engine only) (no socsim deps; engine-free; library-only)
