...
- `socsim-packs`, `socsim-net`, and `socsim-grid` sit beside the engine layer and are orthogonal to it; `socsim-grid` depends only on `socsim-core`. `socsim-packs` depends on `socsim-net` (HR/opinion networks) and `socsim-mechanisms` (the opinion-dynamics mechanisms). See the [module pack catalog](packs.md) for per-pack documentation.
