...
socsim-mechanisms ← general social-dynamics crate: HegselmannKrauseMechanism, DeffuantMechanism, SocialJudgementMechanism, LorenzMechanism, SiContagionMechanism, ThresholdContagionMechanism, AxelrodMechanism, MeanOperator (→ socsim-core only; library-only)
...
- `socsim-mechanisms` is an **orthogonal, optional** crate beside the engine. It depends on **`socsim-core` only** (for the `ScalarOpinions` / `BinaryState` / `CultureVectors` / `Neighbors` capability traits) and is **library-only** — no `ModulePack`, not wired into the `socsim` binary. It is the **general mechanism catalog**: reusable, domain-agnostic building blocks organised into three Cargo **feature families** (all on by default — `opinion-dynamics`, `contagion`, `cultural`), seven mechanisms in total: opinion dynamics (the bounded-confidence `HegselmannKrauseMechanism` and `DeffuantMechanism`, the `SocialJudgementMechanism`, and the `LorenzMechanism`, with the A/G/H/P/R `MeanOperator` family), network contagion (`SiContagionMechanism`, `ThresholdContagionMechanism`), and cultural dissemination (`AxelrodMechanism`) — distinct from the scenario-specific `socsim-hr-lifecycle` crate.
