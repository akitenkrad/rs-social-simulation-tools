...
- **`socsim-survey`**（依存：`csv`/`serde`；**`socsim-*` 依存なし**）は，サーベイ replication が本来ならハードコードする年ごとの ANES recode を，データ駆動の `SurveySchema` —— 人口統計ごとの列名＋値→ラベル valmap，アウトカム（投票）マップ，年齢ビン —— に置き換えます．組み込みの `anes_2012` / `anes_2016` / `anes_2020` スキーマビルダが V 変数の列名と値コードをそのまま移植し，`SurveySchema` 構造体自体が新しいサーベイ向けの拡張 API です（CES も同じ手順で配線できます）．スキーマを与えれば，汎用の `recode_row` / `demo_label` / `actual_outcome` / `estimate_distributions` 関数が recode を行います．
