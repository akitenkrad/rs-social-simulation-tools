...
- **`socsim-datasets`**（`SurveySchema` / `DemoVar` / `ValMap` / `AgeBins` / `OutcomeMap` 型のため **`socsim-survey` のみ**に依存）は，サーベイ replication の*データセット固有*側の単一の真実源です．データセットのスキーマ —— 正規の `anes_2012` / `anes_2016` / `anes_2020` `SurveySchema` ビルダ（V 変数の列名と値コードそのまま，`socsim-survey` から原文どおり移動）と，メタデータのみの CES 2022 スタブ —— と，機械可読のレジストリ（`DatasetMeta` / `DataFile` / `Source` レコード．DOI，ソース URL，引用，ライセンス，検証用のファイルごとの `sha256` / `expect_rows` を保持；`Source` は `Dataverse { base, file_id }` | `Url { url }` | `Manual { instructions_url }`，`Source::download_url()` は Dataverse のアクセス URL を組み立てるか直接 URL を返し，`Manual` では `None`）を保持します．オプションの **`acquire` feature**（`ureq` ＋ `sha2` ＋ `csv` ＋ `tempfile` ＋ `anyhow` を取り込む）は，`fetch()`（ソース URL から消費側リポジトリの gitignore された `data/` ディレクトリへダウンロードし，アトミックに書き込み，`sha256` ＋ 行数を検証し，キャッシュヒットはスキップ）と `raw_to_csv()`（パイプ区切りの raw→CSV コンバータをバイト単位で一致させた Rust 移植）を追加します．データはリポジトリに**決して同梱せず**，ライセンス制約のあるファイル（生の ANES Time Series マイクロデータは無料の electionstudies.org アカウント＋データ利用同意が必要；CES 2022 は Harvard Dataverse の規約経由）は自動ダウンロードせず `Source::Manual` として宣言するため，現時点で登録済みのデータセットはすべて manual です．
