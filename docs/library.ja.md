...
## サーベイ recode と再現ハーネス（`socsim-survey`，`socsim-reproduce`）

ライブラリモードでのサーベイデータ replication を支える，さらに2つのリーフクレートがあります．いずれもエンジン非依存で，論文固有のデータは一切持ちません．

- **`socsim-survey`**（依存：`csv`/`serde`；**`socsim-*` 依存なし**）は，汎用かつ設定駆動のサーベイマイクロデータ recode エンジンです．サーベイ replication が本来ならハードコードする年ごとの recode を，データ駆動の `SurveySchema` —— 人口統計ごとの列名＋値→ラベル valmap，アウトカムマップ，年齢ビン —— に置き換えます．`SurveySchema` 構造体自体が新しいサーベイ向けの拡張 API で，スキーマを与えれば汎用の `recode_row` / `demo_label` / `actual_outcome` / `estimate_distributions` 関数が recode を行います．組み込みの `anes_2012` / `anes_2016` / `anes_2020` スキーマビルダ（V 変数の列名と値コードそのまま）はオプションの **`anes` feature** の背後に置かれ，クレートのデフォルトは汎用エンジンのみです．
