...
- **`socsim-datasets`** (depends on **`socsim-survey` only**, for the `SurveySchema` / `DemoVar` / `ValMap` / `AgeBins` / `OutcomeMap` types) is the single source of truth for the *dataset-specific* side of survey replications. It holds the dataset schemas — the canonical `anes_2012` / `anes_2016` / `anes_2020` `SurveySchema` builders (the exact V-variable columns and value codes, moved here verbatim from `socsim-survey`) plus a metadata-only CES 2022 stub — and a machine-readable registry: `DatasetMeta` / `DataFile` / `Source` records carrying DOI, source URL, citation, license, and per-file `sha256` / `expect_rows` for verification (`Source` is `Dataverse { base, file_id }` | `Url { url }` | `Manual { instructions_url }`; `Source::download_url()` builds the Dataverse access URL or returns the direct URL, `None` for `Manual`). An optional **`acquire` feature** (pulling in `ureq` + `sha2` + `csv` + `tempfile` + `anyhow`) adds `fetch()` (download from source URLs into a consuming repo's gitignored `data/` dir, atomic-write, verify `sha256` + row counts, skip cache hits) and `raw_to_csv()` (a byte-parity Rust port of the pipe-delimited raw→CSV converter). Data is **never vendored** into the repo; license-gated files (raw ANES Time Series microdata needs a free electionstudies.org account + data-use agreement; CES 2022 via Harvard Dataverse terms) are declared `Source::Manual` rather than auto-downloaded, so every registered dataset is currently manual.
