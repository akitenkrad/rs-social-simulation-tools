...
## Survey recode and reproduction harnesses (`socsim-survey`, `socsim-reproduce`)

Two further leaf crates support survey-data replications in library mode; both are engine-free and ship no paper-specific data.

- **`socsim-survey`** (deps: `csv`/`serde`; **no `socsim-*` deps**) is a generic, config-driven survey microdata recode engine: the per-year recode a survey replication would otherwise hard-code becomes a data-driven `SurveySchema` — per-demographic column names + value→label valmaps, an outcome map, and age bins. The `SurveySchema` struct *is* the extension API for new surveys, and given a schema the generic `recode_row` / `demo_label` / `actual_outcome` / `estimate_distributions` functions do the recoding. Built-in `anes_2012` / `anes_2016` / `anes_2020` schema builders (the exact V-variable columns and value codes) ship behind the optional **`anes` feature**; the crate's default is the generic engine only.
