...
- **`socsim-survey`** (deps: `csv`/`serde`; **no `socsim-*` deps**) turns the per-year ANES recode a survey replication would otherwise hard-code into a data-driven `SurveySchema` — per-demographic column names + value→label valmaps, an outcome (vote) map, and age bins. Built-in `anes_2012` / `anes_2016` / `anes_2020` schema builders port the exact V-variable columns and value codes, and the `SurveySchema` struct *is* the extension API for new surveys (CES is wired the same way). Given a schema, the generic `recode_row` / `demo_label` / `actual_outcome` / `estimate_distributions` functions do the recoding.
