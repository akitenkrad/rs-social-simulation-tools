...
- **Zero-dependency distribution comparison** (`socsim_metrics::distribution`, always compiled): `kl_divergence(p, q)` (Kullback–Leibler divergence in nats; inputs treated as weights and normalized internally, epsilon-smoothed to guard against `log(0)`) and `chi_square_homogeneity(observed, expected)` returning `(statistic, p_value)` — the Pearson `χ²` statistic plus an upper-tail p-value from a hand-rolled regularized upper incomplete gamma (also exposed directly as `chi_square_sf`). For *ordinal* distributions it adds `wasserstein_1d(p, q)` (1-D earth-mover distance), `nemd(p, q, range)` (normalized EMD), and the signed `mean_diff` (MD) / `sd_diff` (SDD) location/scale gaps. Canonical distribution-comparison statistics for matching simulated against observed distributions.
- **Zero-dependency rater agreement** (`socsim_metrics::agreement`, always compiled): contingency-table agreement between two categorizations (e.g. a human label vs a model label) — `tetrachoric(n00, n01, n10, n11)` (2×2 latent correlation via the `bvn_cdf` bivariate-normal quadrature), `cohen_kappa`, `icc` / `average_icc` (intraclass correlation), `cramers_v` over an `r×c` table, `prop_agree`, and `prop_test(x1, n1, x2, n2)` (two-proportion `χ²` + p-value).
...
## Survey recode, dataset registry, and reproduction harnesses (`socsim-survey`, `socsim-datasets`, `socsim-reproduce`)

Three further leaf crates support survey-data replications in library mode; all are engine-free and ship no paper-specific data (and `socsim-datasets` never vendors the datasets themselves).

- **`socsim-survey`** (deps: `csv`/`serde`; **no `socsim-*` deps**) is a generic, config-driven survey microdata recode engine: the per-year recode a survey replication would otherwise hard-code becomes a data-driven `SurveySchema` — per-demographic column names + value→label valmaps, an outcome map, and age bins. The `SurveySchema` struct *is* the extension API for new surveys, and given a schema the generic `recode_row` / `demo_label` / `actual_outcome` / `estimate_distributions` functions do the recoding. It is now purely the generic engine — the built-in ANES schemas (and the old `anes` feature) now live in **`socsim-datasets`**.
- **`socsim-datasets`** (depends on **`socsim-survey` only**, for the `SurveySchema` / `DemoVar` / `ValMap` / `AgeBins` / `OutcomeMap` types) is the single source of truth for the *dataset-specific* side of survey replications. It holds the dataset schemas — the canonical `anes_2012` / `anes_2016` / `anes_2020` `SurveySchema` builders (the exact V-variable columns and value codes, moved here verbatim from `socsim-survey`) plus a `ces_2022` builder declaring the native CES codings (`race` 8-category, `gender4` 4-category, `ideo5` 5-point with code 6 "Not sure" unmapped) and a fixed-coded policy outcome `CC22_332a` — and a machine-readable registry: `DatasetMeta` / `DataFile` / `Source` records carrying DOI, source URL, citation, license, and per-file `sha256` / `expect_rows` for verification (`Source` is `Dataverse { base, file_id }` | `Url { url }` | `Manual { instructions_url }`; `Source::download_url()` builds the Dataverse access URL or returns the direct URL, `None` for `Manual`). An optional **`acquire` feature** (pulling in `ureq` + `sha2` + `csv` + `tempfile` + `anyhow`) adds `fetch()` (download from source URLs into a consuming repo's gitignored `data/` dir, atomic-write, verify `sha256` + row counts, skip cache hits) and `raw_to_csv()` (a byte-parity Rust port of the pipe-delimited raw→CSV converter). Data is **never vendored** into the repo; the raw ANES Time Series microdata is license-gated (needs a free electionstudies.org account + data-use agreement), so it is declared `Source::Manual` rather than auto-downloaded, whereas the CES 2022 Common Content is CC0 1.0 (public domain) and is a `Source::Dataverse` that downloads directly from the Harvard Dataverse access API (no account / terms). So only the ANES Time Series datasets are manual; CES 2022 is auto-downloadable.
- **`socsim-reproduce`** (depends on **`socsim-results` only**, for CSV I/O) is a paper-anchor PASS/off reproduction harness. A reproduction run does **not** re-run generation: it reads cached observed values and classifies them against the paper's reference values ("anchors"), emitting a machine-readable PASS / off / NO_DATA summary. The crate ships the mechanics (`Anchor`, `compare_anchor`, `build_rows`, `write_reproduce_summary` / `write_paper_anchors`, `find_latest`) but **no** anchor values — each paper supplies its own `&[Anchor]` slice plus an observation-lookup closure, so the paper's reference numbers stay in the paper's crate.
