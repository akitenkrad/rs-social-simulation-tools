...
`deffuant` は，汎用の `socsim-social-dynamics` パックにおける有界信頼（BC）ファミリーの
...
あるいはフォールバックとしてワールドのロスター）と `opinion_neighbors(i)`（`i` を除く）から
...
| `opinion_neighbors(i)`（`OpinionNeighbors`） | ✓ | | `i` の候補相手（自分自身は除外）． |
