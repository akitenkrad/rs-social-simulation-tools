...
- $\lambda$（`lambda_learn`）— 学習率；$\lambda$ が大きいほど上限に早く到達する．
- $\pi$（`Employee.productivity`）— このステップにおける実効生産性寄与．

$\text{tenure} = 0$ のとき $\pi = 0$；$\text{tenure} \to \infty$ のとき $\pi \to \theta$．
1ヶ月あたりの限界利得は幾何学的に縮小し，経験的な凹型学習曲線を再現します．
Bahk & Gort (1993) は工場レベルの学習を資本・労働・組織コンポーネントに分解しており，
$\lambda = 0.15$ は彼らの報告範囲の中点を業界平均のデフォルトとして設定しています．
...
従業員ごとの $\theta$ と `tenure` のみに依存するため，他の Environment フェーズメカニズムとの順序制約はありません．
...
生き残った各従業員の `productivity` は $\theta$ に向かう凹型曲線に沿って上昇します．
