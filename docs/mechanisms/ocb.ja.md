...
> 満足度が高く組織に適合した従業員が，チームに自発的な知識を提供し，
> 毎ステップ，チームの知識ストックを積み上げます．
> **フェーズ：** Interaction．**出典：** calibration（α_k チューナブル）．**種別：** tunable．
...
正式な職務要件を超えて従業員が貢献する，裁量的・役割外の努力です．
HR ライフサイクルモジュールでは，この行動は自発的な知識共有として操作化されています：
毎ステップ，全従業員が満足度と組織適合度に比例した貢献をチームの `knowledge_stock` に行います．
不満足または不適合の従業員はほとんど，あるいは全く貢献せず；
満足度が高く組織に適合した従業員はチームの集合的な知識成長を意義あるかたちで加速させます．

このメカニズムは単一の経験的研究に紐付けるのではなく，意図的にキャリブレーションベースとしています：
スケールファクター `α_k` は，`knowledge_loss` が課す知識損失に対して知識蓄積のバランスを取るための
自由パラメータです．

## 2. 理論と出典

OCB は，態度的な成果（満足度，組織コミットメント，適合）が役割外行動を予測することを示す
幅広い文献と関連しています（Organ, 1988; Kristof-Brown et al., 2005）．
socsim の実装はこの貢献を線形積として扱います：

```text
ΔK_team += α_k · satisfaction · po_fit        (for each employee on that team)
```

- `satisfaction` — `fit` メカニズムによって毎ステップ更新される従業員の現在の職務満足度 ∈ [0, 1]．
- `po_fit` — person–organisation fit ∈ [0, 1]；組織の価値観と同一視する従業員ほど多く貢献する．
- `α_k`（`alpha_k`）— チューナブルなスケーリング係数（デフォルト 0.30）；
  モデラーの目標チャーン率に対して `knowledge_stock` の長期的な推移を `knowledge_loss` とバランスさせるために設定する．

従業員は昇順の `AgentId` 順で処理されるため，`ΔK_team` への浮動小数点累積は
実行をまたいで完全に決定論的です．
...
- **下流：** `knowledge_loss`（PostStep）は `Team.knowledge_stock` を読み取り，
  離職による暗黙知の損失を適用します；`ocb` マイナス `knowledge_loss` の純効果が，
  ストックが成長するか減少するかを決定します．
- `peer_effect` への**依存関係なし**；2つは Interaction フェーズを共有しますが，
  異なるフィールドに書き込みます．
...
`knowledge_stock` は毎ステップ着実に成長するはずです．
離職が少なければ `knowledge_loss` は小さく，ストックは上昇傾向を示します．
高チャーンシナリオ（例：有害行動の蔓延エピソード）では，満足度が低下し，
OCB 貢献が縮小して，`knowledge_loss` が OCB 獲得を超える可能性があり，
ストックは減少します．`alpha_k` を上げるとベースライン成長率が上がり，
